...
base = "C:/Cloud/Dropbox/Work/Axin/Python/ApplyDataModel/"
fgdbs = ['rdt.sde']
...
for fgdb in fgdbs:
    ...
    tblsheet = workbook.create_sheet(f"TablesFields {fgdb[:-4]}")
    print(base+fgdb)
    ...
    for tbl in arcpy.ListTables():
        ...
        if tbl not in all_fcs_and_tables:
            all_fcs_and_tables.append(shrttbl)
        
        if tbl not in geodb_fcs[fgdb]:
            geodb_fcs[fgdb].append(shrttbl)

        sheet[f"{cols[colcount]}{rowcount}"] = shrttbl
        rowcount += 1
        
        tblsheet[f"{cols[tblcolcount]}{tblsubrowcount}"].font = fonts[0]
        tblsheet[f"{cols[tblcolcount]}{tblsubrowcount}"] = shrttbl
        tblsubrowcount += 1
        ...
        for fd in  desc.fields:
            if not fd_header:
                for k,an_attr in enumerate(fieldnames):
                    tblsheet[f"{cols[k]}{tblsubrowcount}"] = an_attr
                fd_header = True
                tblsubrowcount += 1 
            for k,an_attr in enumerate(fieldnames):
                tblsheet[f"{cols[k]}{tblsubrowcount}"] = getattr(fd,an_attr)
